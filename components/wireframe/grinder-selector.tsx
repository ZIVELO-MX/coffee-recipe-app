...
export function GrinderSelector({ selected, onSelect, onClose }: {
  ...
  }, [onClose, portalTarget, requestClose])
  useEffect(() => {
  ...
  return (
    <>
      ...
      {createPortal(<div className={`grinder-overlay fixed inset-0 z-[100] flex items-center justify-center bg-background/70 p-4 backdrop-blur-sm ${closing ? "grinder-overlay-closing" : ""}`} onClick={requestClose}>
      <div ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="grinder-title" className={`grinder-dialog glass-strong flex max-h-[80vh] w-full max-w-[400px] flex-col rounded-[2rem] outline-none ${closing ? "grinder-dialog-closing" : ""}`} onClick={(event) => event.stopPropagation()}>
        ...
        <div className="flex items-center justify-between p-5 pb-3">
          ...
          <button type="button" onClick={requestClose} aria-label="Cerrar" className="flex h-9 w-9 items-center justify-center rounded-full bg-secondary text-muted-foreground"><X className="h-4 w-4" aria-hidden="true" /></button>
        </div>
            ...
            <div key={brand} className="pt-3">
              ...
              <div className="flex flex-col gap-1.5">
                {models?.map((grinder) => {
                  ...
                  return <button key={grinder.id} data-grinder-id={grinder.id} type="button" disabled={closing} onClick={handleGrinderSelect} className={`flex w-full items-center justify-between rounded-2xl border px-4 py-3.5 text-left text-sm transition-colors disabled:pointer-events-none disabled:opacity-60 ${isSelected ? "border-primary/50 bg-primary/10 text-foreground" : "border-border bg-card text-foreground hover:bg-secondary/50"}`}>{grinder.name}{isSelected && <Check className="h-4 w-4 text-primary" aria-hidden="true" />}</button>
                })}
              </div>
            </div>
          ))}
